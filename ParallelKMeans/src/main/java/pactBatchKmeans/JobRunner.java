...
import eu.stratosphere.client.program.*;
...
import eu.stratosphere.nephele.runtime.JobSubmissionException;
...
public class JobRunner {
  ...
  public void runLocal(Plan plan) throws Exception {
    ...
    try {
      stop.start();
      netRuntime = executor.executePlan(plan).getNetRuntime();
      stop.stop();
    } catch (Exception e) {
      executor.stop();
      throw e;
 ...
 * @throws JobInstantiationException 
   */
  public void run(String jarPath, String assemblerClassName, String[] jobArgs, String configPath, String jobManagerAddress, String jobManagerPort, boolean waitForCompletion) throws IOException, JobExecutionException, JobExecutionException, JobSubmissionException, JobInstantiationException {

    File jar = new File(jarPath);

    try {

      // PackagedProgram wraps plan related function
      // Pass (classname and) arguments here
      // PackagedProgram(File jarFile, String... args)
      // PackagedProgram(File jarFile, String className, String... args)
      PackagedProgram prog = null;
      if (assemblerClassName.equals("")) {
        prog = new PackagedProgram(jar, jobArgs);
      } else {
        prog = new PackagedProgram(jar, assemblerClassName, jobArgs);
      }
      ...
      // Client.run(...)
      //  - compiles the PackagedProgram to a OptimizedPlan using
      ...
      final Stopwatch stop = new Stopwatch();
      stop.start();
      client.run(prog.getPlanWithJars(), waitForCompletion);
      stop.stop();
  ...
  private void printPlanJson(PackagedProgram program, Client client) throws JobExecutionException, IOException, JobSubmissionException, JobInstantiationException {
 ...
 * @throws JobInstantiationException 
   */
  public void runFromPropertyFile(String propertyFile, boolean waitForCompletion) throws IOException, JobExecutionException, JobSubmissionException, JobInstantiationException {
